...
def main():
    parser = argparse.ArgumentParser(description='Generate random mask in 256x256')
    parser.add_argument('--dest', type=str, default='./masks', help='destination folder')
    parser.add_argument('--n', type=int, default=30000, help='number of masks')
    ...
    for i in range(num):
        ...
        # save mask
        img_mask = Image.fromarray(mask[0] * 255)
        img_mask = img_mask.convert('RGB')
        filename = f'{dest_folder}/{str(i).zfill(5)}.png'
        img_mask.save(filename)
        print(f'Saved {filename}')


# example usage: python gen_mask_256.py --dest ./masks --n 30000
